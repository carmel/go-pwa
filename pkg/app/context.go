...
func (ctx Context) DeviceID() string {
	var id string
	if err := ctx.localStorage.Get("/go-pwa/deviceID", &id); err != nil {
	...
	id = uuid.NewString()
	if err := ctx.localStorage.Set("/go-pwa/deviceID", id); err != nil {
...
// Async initiates a function asynchronously. It enables go-pwa to monitor
